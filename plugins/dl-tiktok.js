...
async function getTikTokVideo(url) {
    ...
    const sdLink = $("a:contains('Download MP4 [1]'), a:contains('MP4 [1]')").first().attr("href") || null;
  
    const hdLink = $("a:contains('Download MP4 HD'), a:contains('MP4 HD'), a[href*='-hd']").first().attr("href") || null;

    return { title, videoLink: sdLink || hdLink };
}
...
cmd({
    ...
    try {
        ...
        const videoRes = await axios.get(data.videoLink, {
