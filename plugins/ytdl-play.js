...
// ── API pool — har request next available API legi
const apiPool = [
    async (videoUrl) => {
...
// ── Round robin counter — har request alag API se start karegi
let apiCounter = 0;

const downloadAudio = async (videoUrl) => {
    const total = apiPool.length;
    const startIndex = apiCounter % total;
    apiCounter++;

    for (let i = 0; i < total; i++) {
        const index = (startIndex + i) % total;
        try {
            const buffer = await apiPool[index](videoUrl);
            return buffer;
        } catch (e) {
