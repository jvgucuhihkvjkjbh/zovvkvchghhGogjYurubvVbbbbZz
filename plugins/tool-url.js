...
cmd({
  ...
  try {
    ...
    if (!imgbbResponse.data || !imgbbResponse.data.success || !imgbbResponse.data.data) {
      throw (imgbbResponse.data?.error?.message || "Failed to upload to ImgBB");
    }

    const result = imgbbResponse.data.data;
    const mediaUrl = result.url || result.display_url || result.image?.url;
