...
async function groupStatus(sock, jid, rawContent, useAiBadge = false, customName = "ADEEL-MD", customEmoji = "🕷️") {
  ...
  if (isPreGenerated) {
    ...
    if (content.image && !innerMsg.imageMessage) {
      throw new Error('Image upload/generation failed — imageMessage was not produced. Check media size/format and network access to WhatsApp media servers.');
    }
    if (content.video && !innerMsg.videoMessage) {
      throw new Error('Video upload/generation failed — videoMessage was not produced. Check media size/format and network access to WhatsApp media servers.');
    }
    if (content.audio && !innerMsg.audioMessage) {
      throw new Error('Audio upload/generation failed — audioMessage was not produced. Check media size/format and network access to WhatsApp media servers.');
    }
  ...
  if (!msgKey) {
    throw new Error('Could not determine message type after generation — status content is empty or malformed.');
  }
...
cmd({
    ...
    try {
      ...
      if (type === 'txt') {
        ...
        // 1) Preferred: framework-provided quoted.download() helper, if available
        if (quoted && typeof quoted.download === 'function') {
          try {
            buffer = await quoted.download();
          } catch (dlErr) {
            console.error('[SWGC] quoted.download() failed, will try fallback:', dlErr.message);
            buffer = null;
          }
        }

        // 2) Fallback: manual downloadMediaMessage using best-available key info
        if (!buffer) {
          const contextInfo =
          ...
          try {
            ...
            throw new Error(`Media download failed: ${dlErr.message}`);
          }
        }

        if (!buffer || !Buffer.isBuffer(buffer) || buffer.length === 0) {
          throw new Error('Downloaded media is empty — could not fetch the quoted media. Try replying again or resend the media.');
        }
