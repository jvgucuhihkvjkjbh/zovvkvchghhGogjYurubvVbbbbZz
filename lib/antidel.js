...
const DeletedText = async (conn, mek, jid, deleteInfo, isGroup, update) => {
    ...
    await conn.sendMessage(jid, {
        text: deleteInfo,
        contextInfo: { mentionedJid: mentionList.filter(Boolean) },
    }, { quoted: mek });
};
    ...
    for (const update of updates) {
        ...
        if (update.update.message === null) {
            try {
                ...
                if (isStatus) {
                    // ✅ STATUS - fix: continue ہٹایا، media بھی relay ہوگی
                    const senderJid = mek.key?.participant || mek.key?.remoteJid || update.key?.participant || update.key?.remoteJid;
...
*╰──────────────────────*`;
                    ...
                    jid = conn.user.id.split(':')[0] + '@s.whatsapp.net';
                    const mentionList = [senderJid, deleterJid].filter(Boolean);
                    ...
                    if (mek.message?.conversation || mek.message?.extendedTextMessage) {
                        ...
                        try {
                            ...
                            await conn.sendMessage(jid, { text: `_Media could not be recovered._` });
                        }
                    }
                    continue;
